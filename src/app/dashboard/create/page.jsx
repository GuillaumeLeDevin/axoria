...
export default function page() {
    ...
    async function handleSubmit(event) {
        ...
        submitButtonRef.current.textContent = "Creating post..."
        submitButtonRef.current.disabled = true

        try {
            const result = await addPost(formData)
            if(result.success) {
                submitButtonRef.current.textContent = "Post created! ✅"
...
{/* ---------------------- Content ---------------------- */}
            ...
            placeholder="Content"
            required
            className='min-h-44 text-xl appearance-none w-full border rounded shadow p-8 text-gray-700 mb-4 focus:outline-slate-400'></textarea>
